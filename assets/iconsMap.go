...
var Icon_Set = map[string]*Icon_Info{
	...
	"css":              {i: "󰌜", c: [3]uint8{66, 165, 245}},  // css
	...
	"xml":              {i: "󰗀", c: [3]uint8{64, 153, 69}},   // xml
	...
	"test-jsx":         {i: "󰂖", c: [3]uint8{35, 188, 212}},  // test-jsx
	"test-js":          {i: "󰂖", c: [3]uint8{255, 202, 61}},  // test-js
	...
	"typescript-def":   {i: "\ue628", c: [3]uint8{3, 136, 209}},   // typescript-def
	"test-ts":          {i: "󰂖", c: [3]uint8{3, 136, 209}},   // test-ts
	"pdf":              {i: "\uf1c1", c: [3]uint8{244, 68, 62}},   // pdf
	"table":            {i: "󰈛", c: [3]uint8{139, 195, 74}},  // table
	...
	"csharp":           {i: "󰌛", c: [3]uint8{2, 119, 189}},   // csharp
	"zip":              {i: "\uf410", c: [3]uint8{175, 180, 43}},  // zip
	"exe":              {i: "\uf2d0", c: [3]uint8{229, 77, 58}},   // exe
	"java":             {i: "\ue738", c: [3]uint8{244, 68, 62}},   // java
	"c":                {i: "󰙱", c: [3]uint8{2, 119, 189}},   // c
	"cpp":              {i: "󰙲", c: [3]uint8{2, 119, 189}},   // cpp
	"go":               {i: "󰟓", c: [3]uint8{32, 173, 194}},  // go
	"go-mod":           {i: "󰟓", c: [3]uint8{237, 80, 122}},  // go-mod
	"go-test":          {i: "󰟓", c: [3]uint8{255, 213, 79}},  // go-test
	"python":           {i: "󰌠", c: [3]uint8{52, 102, 143}},  // python
	"python-misc":      {i: "󰌠", c: [3]uint8{130, 61, 28}},   // python-misc
	"url":              {i: "󰌷", c: [3]uint8{66, 165, 245}},  // url
	"console":          {i: "󰆍", c: [3]uint8{250, 111, 66}},  // console
	"word":             {i: "󰈬", c: [3]uint8{1, 87, 155}},    // word
	"certificate":      {i: "󱆆", c: [3]uint8{249, 89, 63}},   // certificate
	"key":              {i: "󱆄", c: [3]uint8{48, 166, 154}},  // key
	...
	"swift":            {i: "󰛥", c: [3]uint8{249, 95, 63}},   // swift
	"docker":           {i: "\uf308", c: [3]uint8{1, 135, 201}},   // docker
	"powerpoint":       {i: "\f3b088a7", c: [3]uint8{209, 71, 51}},   // powerpoint
	"video":            {i: "\uf1c8", c: [3]uint8{253, 154, 62}},  // video
	"virtual":          {i: "\uf109", c: [3]uint8{3, 155, 229}},   // virtual
	"email":            {i: "󰇰", c: [3]uint8{66, 165, 245}},  // email
	"audio":            {i: "\uf001", c: [3]uint8{239, 83, 80}},   // audio
	"coffee":           {i: "\uf0f4", c: [3]uint8{66, 165, 245}},  // coffee
	"document":         {i: "󰈙", c: [3]uint8{66, 165, 245}},  // document
	"rust":             {i: "\ue7a8", c: [3]uint8{250, 111, 66}},  // rust
	"raml":             {i: "\ue60b", c: [3]uint8{66, 165, 245}},  // raml
	"xaml":             {i: "󰙳", c: [3]uint8{66, 165, 245}},  // xaml
	...
	"smarty":           {i: "󰌵", c: [3]uint8{255, 207, 60}},  // smarty
	"stylus":           {i: "\ue600", c: [3]uint8{192, 202, 51}},  // stylus
	"verilog":          {i: "󰘚", c: [3]uint8{250, 111, 66}},  // verilog
	"robot":            {i: "󰚩", c: [3]uint8{249, 89, 63}},   // robot
	"solidity":         {i: "󰡪", c: [3]uint8{3, 136, 209}},   // solidity
	"yang":             {i: "󰚀", c: [3]uint8{66, 165, 245}},  // yang
	"vercel":           {i: "\uf47e", c: [3]uint8{207, 216, 220}}, // vercel
	"applescript":      {i: "\uf302", c: [3]uint8{120, 144, 156}}, // applescript
	"cake":             {i: "󰃫", c: [3]uint8{250, 111, 66}},  // cake
	...
	"webpack":          {i: "\ue6a3", c: [3]uint8{142, 214, 251}}, // webpack
	...
	"tune":             {i: "󰙪", c: [3]uint8{251, 193, 60}},  // tune
	"contributing":     {i: "󰅍", c: [3]uint8{255, 202, 61}},  // contributing
	"readme":           {i: "󰋼", c: [3]uint8{66, 165, 245}},  // readme
	"changelog":        {i: "\uf4ed", c: [3]uint8{139, 195, 74}},  // changelog
	"credits":          {i: "󰉠", c: [3]uint8{156, 204, 101}}, // credits
	...
	"conduct":          {i: "󰅎", c: [3]uint8{205, 220, 57}},  // conduct
	"jenkins":          {i: "\ue767", c: [3]uint8{240, 214, 183}}, // jenkins
	"code-climate":     {i: "󰔉", c: [3]uint8{238, 238, 238}}, // code-climate
	...
	"azure":            {i: "󰠅", c: [3]uint8{31, 136, 229}},  // azure
	"razor":            {i: "󰁥", c: [3]uint8{66, 165, 245}},  // razor
	"asciidoc":         {i: "󰈙", c: [3]uint8{244, 68, 62}},   // asciidoc
	"edge":             {i: "\uf1fa", c: [3]uint8{239, 111, 60}},  // edge
	"scheme":           {i: "", c: [3]uint8{244, 68, 62}},   // scheme
	"3d":               {i: "\ue79b", c: [3]uint8{40, 182, 246}},  // 3d
	"svg":              {i: "󰜡", c: [3]uint8{255, 181, 62}},  // svg
	"vim":              {i: "\ue62b", c: [3]uint8{67, 160, 71}},   // vim
	"moonscript":       {i: "\uf186", c: [3]uint8{251, 193, 60}},  // moonscript
	"codeowners":       {i: "󰀈", c: [3]uint8{175, 180, 43}},  // codeowners
	"disc":             {i: "\ue271", c: [3]uint8{176, 190, 197}}, // disc
	"fortran":          {i: "F", c: [3]uint8{250, 111, 66}},       // fortran
	"tcl":              {i: "󰛓", c: [3]uint8{239, 83, 80}},   // tcl
	"liquid":           {i: "\ue275", c: [3]uint8{40, 182, 246}},  // liquid
	"prolog":           {i: "\ue7a1", c: [3]uint8{239, 83, 80}},   // prolog
	"husky":            {i: "", c: [3]uint8{229, 229, 229}}, // husky
	"coconut":          {i: "󰃓", c: [3]uint8{141, 110, 99}},  // coconut
	"sketch":           {i: "󰇈", c: [3]uint8{255, 194, 61}},  // sketch
	"pawn":             {i: "\ue261", c: [3]uint8{239, 111, 60}},  // pawn
	"commitlint":       {i: "󰜘", c: [3]uint8{43, 150, 137}},  // commitlint
	"dhall":            {i: "\uf448", c: [3]uint8{120, 144, 156}}, // dhall
	"dune":             {i: "󰔉", c: [3]uint8{244, 127, 61}},  // dune
	"shaderlab":        {i: "󰚯", c: [3]uint8{25, 118, 210}},  // shaderlab
	"command":          {i: "󰘳", c: [3]uint8{175, 188, 194}}, // command
	"stryker":          {i: "\uf05b", c: [3]uint8{239, 83, 80}},   // stryker
	"modernizr":        {i: "\ue720", c: [3]uint8{234, 72, 99}},   // modernizr
	"roadmap":          {i: "󰛘", c: [3]uint8{48, 166, 154}},  // roadmap
	...
	"routing":          {i: "󰙁", c: [3]uint8{67, 160, 71}},   // routing
	"laravel":          {i: "\ue73f", c: [3]uint8{248, 80, 81}},   // laravel
	"pug":              {i: "\ue60e", c: [3]uint8{239, 204, 163}}, // pug (Not supported by nerdFont)
	"blink":            {i: "󰸬", c: [3]uint8{249, 169, 60}},  // blink (The Foundry Nuke) (Not supported by nerdFont)
	"postcss":          {i: "󰌜", c: [3]uint8{244, 68, 62}},   // postcss (Not supported by nerdFont)
	...
	"vala":             {i: "󰊬", c: [3]uint8{149, 117, 205}}, // vala (Not supported by nerdFont)
	...
	"powershell":       {i: "󰞷", c: [3]uint8{5, 169, 244}},   // powershell (Not supported by nerdFont)
	"gradle":           {i: "\ue660", c: [3]uint8{29, 151, 167}},  // gradle (Not supported by nerdFont)
	"arduino":          {i: "\ue255", c: [3]uint8{35, 151, 156}},  // arduino (Not supported by nerdFont)
	"tex":              {i: "󰊄", c: [3]uint8{66, 165, 245}},  // tex (Not supported by nerdFont)
	...
	"actionscript":     {i: "󰘦", c: [3]uint8{244, 68, 62}},   // actionscript (Not supported by nerdFont)
	"autohotkey":       {i: "󰌓", c: [3]uint8{76, 175, 80}},   // autohotkey (Not supported by nerdFont)
	"flash":            {i: "󰉁", c: [3]uint8{198, 52, 54}},   // flash (Not supported by nerdFont)
	"swc":              {i: "󰛕", c: [3]uint8{198, 52, 54}},   // swc (Not supported by nerdFont)
	...
	"puppet":           {i: "󰂖", c: [3]uint8{251, 193, 60}},  // puppet (Not supported by nerdFont)
	"purescript":       {i: "󰅱", c: [3]uint8{66, 165, 245}},  // purescript (Not supported by nerdFont)
	...
	"babel":            {i: "󰂡", c: [3]uint8{253, 217, 59}},  // babel (Not supported by nerdFont)
	...
	"eslint":           {i: "󰛸", c: [3]uint8{121, 134, 203}}, // eslint (Not supported by nerdFont)
	"mocha":            {i: "󰆪", c: [3]uint8{161, 136, 127}}, // mocha (Not supported by nerdFont)
	"firebase":         {i: "\ue787", c: [3]uint8{251, 193, 60}},  // firebase (Not supported by nerdFont)
	"stylelint":        {i: "󰙸", c: [3]uint8{207, 216, 220}}, // stylelint (Not supported by nerdFont)
	"prettier":         {i: "󰏣", c: [3]uint8{86, 179, 180}},  // prettier (Not supported by nerdFont)
	"jest":             {i: "J", c: [3]uint8{244, 85, 62}},        // jest (Not supported by nerdFont)
	"storybook":        {i: "󰠮", c: [3]uint8{237, 80, 122}},  // storybook (Not supported by nerdFont)
	"fastlane":         {i: "󰜀", c: [3]uint8{149, 119, 232}}, // fastlane (Not supported by nerdFont)
	"helm":             {i: "󰠳", c: [3]uint8{32, 173, 194}},  // helm (Not supported by nerdFont)
	"i18n":             {i: "󰊿", c: [3]uint8{121, 134, 203}}, // i18n (Not supported by nerdFont)
	"semantic-release": {i: "󰈐", c: [3]uint8{245, 245, 245}}, // semantic-release (Not supported by nerdFont)
	...
	"tailwindcss":      {i: "󰞍", c: [3]uint8{77, 182, 172}},  // tailwindcss (Not supported by nerdFont)
	"gcp":              {i: "󰅣", c: [3]uint8{70, 136, 250}},  // gcp (Not supported by nerdFont)
	"opam":             {i: "\uf1ce", c: [3]uint8{255, 213, 79}},  // opam (Not supported by nerdFont)
	"pascal":           {i: "󰏛", c: [3]uint8{3, 136, 209}},   // pascal (Not supported by nerdFont)
	...
	"dir-include":     {i: "󰉗", c: [3]uint8{3, 155, 229}},   // dir-include
	"dir-import":      {i: "󰉗", c: [3]uint8{175, 180, 43}},  // dir-import
	"dir-upload":      {i: "󰉙", c: [3]uint8{250, 111, 66}},  // dir-upload
	"dir-download":    {i: "󰉍", c: [3]uint8{76, 175, 80}},   // dir-download
	"dir-secure":      {i: "󰉐", c: [3]uint8{249, 169, 60}},  // dir-secure
	"dir-images":      {i: "󰉏", c: [3]uint8{43, 150, 137}},  // dir-images
	"dir-environment": {i: "󰉏", c: [3]uint8{102, 187, 106}}, // dir-environment
}

// default icons in case nothing can be found
var Icon_Def = map[string]*Icon_Info{
	"dir":        {i: "󰉋", c: [3]uint8{224, 177, 77}},
	"diropen":    {i: "󰝰", c: [3]uint8{224, 177, 77}},
	"hiddendir":  {i: "󰉖", c: [3]uint8{224, 177, 77}},
	"exe":        {i: "󰈔", c: [3]uint8{76, 175, 80}},
	"file":       {i: "󰈤", c: [3]uint8{65, 129, 190}},
	"hiddenfile": {i: "󰘓", c: [3]uint8{65, 129, 190}},
}
